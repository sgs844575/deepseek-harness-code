...
*/
function claudeSpawnSpec(options, graceMs, platform = process.platform) {
...
function successfulResult(message) {
	if (message.subtype !== "success" || message.is_error || message.result.trim().length === 0) {
		const detail = message.subtype === "success" ? "success result was marked as an error or contained no answer" : message.errors.join("; ") || message.subtype;
...
*/
async function consumeClaudeQuery(query) {
	let answer;
	for await (const message of query) {
		if (message.type !== "result") continue;
		answer = successfulResult(message);
	}
	if (answer === void 0) throw new Error("subagent-claude-code: Claude Code ended without a result");
	return {
		output: [{
			type: "text",
			text: answer
...
async function disposeClaudeCodeChild(query, child) {
	...
	if (failures.length === 1 && firstFailure !== void 0) throw firstFailure;
	if (failures.length > 1) throw new AggregateError(failures, "subagent-claude-code: query and process cleanup failed");
}
/**
...
*/
function claudeQueryOptions(spec, controller, capture) {
	return {
...
async function startClaudeCodeRun(request, spec) {
	...
	try {
		...
		if (child !== void 0) try {
			...
			throw new AggregateError([thrown(error), thrown(disposeError)], "subagent-claude-code: startup failed and CLI cleanup also failed");
		}
		else if (query$1 !== void 0) try {
			query$1.close();
		} catch (disposeError) {
			throw new AggregateError([thrown(error), thrown(disposeError)], "subagent-claude-code: startup failed and query cleanup also failed");
		}
		if (cancelledBeforeCleanup || request.signal.aborted) throw new Error("subagent-claude-code: request was aborted before SDK startup");
		throw thrown(error);
	}
	...
	return subprocessRunHandle({
		...
		onAbort,
		requestCancel,
		teardown: () => disposeClaudeCodeChild(publishedQuery, publishedChild)
	});
}
//#endregion
