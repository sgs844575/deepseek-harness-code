...
- **新会话视图（Codex 布局）**：空白会话整组垂直居中（问句 + 项目选择器 +
  输入卡 + 建议卡）；项目选择器在输入卡上方（含「选择文件夹…」选任意地址），
  开始对话后归属锁定、选择器与建议卡消失
- **附件上传**：＋菜单选择文件 → 复制进工作区 `.dsh/uploads/` 并随消息注明
  路径（Agent 文件工具读取；单个 ≤20MiB、每条 ≤8 个、chips 预览）
- **回合完成折叠**：过程（思考/工具/中间回复）默认收进折叠块，只显示最终回复
- **插件设置页**：内置组合行状态 + 自定义插件安装/启停/删除（boot 补丁注入，
  应用变更重启引擎）
- **修复**：会话标题批量拉取（空白回退「新对话」）· 归档闭环（启动跳过已归档/
  归档即切新会话）· 最近区跨项目平铺 · 侧栏行按钮不再压标题 · ＋号最左 ·
  工具栏换行 · 对话流与输入卡同宽 · harness 模块并行预载
